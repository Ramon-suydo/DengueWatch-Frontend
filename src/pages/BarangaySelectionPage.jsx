...
useEffect(() => {
    const saved = localStorage.getItem('denguewatch.districtBarangays');
    if (saved) {
        try {
            const parsed = JSON.parse(saved);
            setBarangays(parsed.sort());
        } catch {
            setBarangays([]);
        }
    }
                ...
                <section className="w-full max-w-2xl rounded-[2rem] bg-paper/95 px-6 py-8 shadow-soft sm:px-8 sm:py-10">
                    <div className="flex flex-col items-center text-center">
                        ...
                        <p className="mt-3 max-w-xl text-sm leading-7 text-ink/70 sm:text-base">
                            Select your barangay in <span className="font-semibold text-navy">{selectedDistrict}</span>,{' '}
                            <span className="font-semibold text-navy">{selectedCity}</span>.
                        </p>
                        <p className="mt-2 text-xs text-ink/50">
                            ⚠️ Risk prediction is currently city-based. Barangay-level coming soon.
                        </p>
                    </div>
                    ...
                    <div className="mt-4 grid max-h-72 gap-3 overflow-y-auto pr-1">
                        {loading ? (
                            <p className="py-4 text-center text-sm text-ink/50">Loading barangays...</p>
                        ) : filtered.length === 0 ? (
